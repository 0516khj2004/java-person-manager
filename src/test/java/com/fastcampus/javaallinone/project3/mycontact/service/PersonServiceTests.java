...
class PersonServiceTests {
    ...
    void getAll(){
        when(personRepository.findAll(any(Pageable.class)))
                .thenReturn(new PageImpl<>(Lists.newArrayList(new Person("koo1"),new Person("koo2"), new Person("koo3"))));

        Page<Person> result = personService.getAll(PageRequest.of(0,3));

        assertThat(result.getNumberOfElements()).isEqualTo(3);
        assertThat(result.getContent().get(0).getName()).isEqualTo("koo1");
        assertThat(result.getContent().get(1).getName()).isEqualTo("koo2");
        assertThat(result.getContent().get(2).getName()).isEqualTo("koo3");
    }
    @Test
